...
div
  ...
  table
    tr
      th {{ $t('visualizations.bucketPage.type') }}
      td {{ bucket.type }}
    tr
      th {{ $t('visualizations.bucketPage.client') }}
      td {{ bucket.client }}
    tr
      th {{ $t('visualizations.bucketPage.hostname') }}
      td {{ bucket.hostname }}
    tr
      th {{ $t('visualizations.bucketPage.created') }}
      td {{ bucket.created | iso8601 }}
    tr(v-if="bucket.metadata")
      th {{ $t('visualizations.bucketPage.firstLastEvent') }}
      td
        | {{ bucket.metadata.start}} /
        | {{ bucket.metadata.end }}
    tr
      th {{ $t('visualizations.bucketPage.eventCount') }}
      td {{ eventcount }}
    tr
      th {{ $t('visualizations.bucketPage.data') }}
      td {{ bucket.data }}
